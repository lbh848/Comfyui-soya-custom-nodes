...
MODULE_PATH = Path(__file__).parents[1] / "soya_char_lora_eye_detailer.py"
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "soya_char_lora_eye_detailer_for_test", MODULE_PATH
    )
...
class CharLoraEyeDetailerCropTests(unittest.TestCase):
    def test_bottom_edge_crop_shifts_inward_and_uses_actual_vae_multiple(self):
        ...
        eye_context = {
            "faces": [
                {
                    "eye_mask": np.ones((30, 24), dtype=np.uint8),
                    "eyebrow_mask": np.zeros((30, 24), dtype=np.float32),
                    "bbox_clamped": [20, 70, 44, 100],
                }
            ]
        }
        char_tags = {
        ...
        self.assertGreater(torch.count_nonzero(mask).item(), 0)
        self.assertEqual(vae.encoded_shapes, [(1, 96, 80, 3)])
        self.assertIn("crop:(0,4,80,100) 80x96", info)
